...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, onLogout, isOpen, setIsOpen }) => {
    ...
    return (
        <>
            ...
            <aside className={`absolute lg:relative inset-y-0 left-0 z-40 w-64 bg-bg-secondary/80 backdrop-blur-lg flex-shrink-0 flex flex-col border-r border-border-color p-4 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                    ...
                    <ul>
                        {navItems.map(({ icon: Icon, label }) => (
                            <li key={label} className="my-1">
                                <a
                                    href="#"
                                    onClick={(e) => { e.preventDefault(); setActivePage(label); setIsOpen(false); }}
