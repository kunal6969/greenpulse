...
import { SAVINGS_LEADERBOARD_DATA } from '../../constants';
...
const PodiumItem: React.FC<{ entry: LeaderboardEntry, index: number }> = ({ entry, index }) => {
    ...
    return (
        ...
        >
            ...
            <p className="text-xl font-bold mt-1 drop-shadow-md">{entry.score.toLocaleString()} <span className="text-sm font-medium opacity-80">GP</span></p>
        </div>
    );
};
...
const GamificationLeaderboard: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const podiumEntries = SAVINGS_LEADERBOARD_DATA.slice(0, 3);
    ...
    <>
      <Card className={`${className} flex flex-col`}>
        ...
        <button 
          ...
          className="mt-4 w-full bg-primary/10 hover:bg-primary/20 text-primary font-bold py-2 px-4 rounded-lg transition-colors duration-300"
        >
      ...
      <Modal title="Full Leaderboard" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="space-y-2 pr-2 max-h-[60vh] overflow-y-auto">
          {SAVINGS_LEADERBOARD_DATA.map((entry, index) => (
            <div
              key={entry.rank}
            ...
            >
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <p className="font-semibold text-primary w-24 text-right">{entry.score.toLocaleString()} GP</p>
                </div>
            </div>
          ))}
        </div>
